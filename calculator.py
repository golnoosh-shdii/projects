...
def calculator():
    b = operator_entry.get()
    try:
        a = float(number1_entry.get())
        c = float(number2_entry.get())
    except:
        ...
        if b == "+":
            f = a + c
            ans_lable.config(text=f"{f}")
        elif b == "-":
            d = a - c
            ans_lable.config(text=f"{d}")
        elif b == "/":
             g = a / c
             ans_lable.config(text=f"{g}")
        elif b == "*":
             z = a * c
             ans_lable.config(text=f"{z}")
        elif b == "**":
            m = math.pow(a, c)
            ans_lable.config(text=f"{m}")
        elif b == "log":
            h = math.log(a, c)
            ans_lable.config(text=f"{h}")
